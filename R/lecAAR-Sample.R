...
# kriging und Isolinien erstellen
 ...
 SLDF <- rasterToContour(applylarge,
                         nlevels = 50)
...
# Punkte neu laden, weil ich mit sp arbeite und ich den schon geschriebenen code nicht in sf umwandeln möchte --> TODO 
 ...
 koordinaten <- as.data.frame(sites@coords)

 
 
 
# Nearest Neighbour 
 sites_nn <- RANN::nn2(koordinaten) 
...
# automatische Ermittlung des "geeigneten Punkts" 
 ...
 flaechenzunahme_plot <- ggplot2::ggplot(data = flaechenzunahme, ggplot2::aes(x = level, y = quotient)) +
   ggplot2::geom_line() +
   ggplot2::geom_vline(ggplot2::aes(xintercept = which(splus2R::peaks(flaechenzunahme$quotient))[1]), col = "red") +
